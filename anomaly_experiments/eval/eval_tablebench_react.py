#!/usr/bin/env python3
"""Run TableBench-style JSONL tasks with the DATAMIND ReAct Python loop.
...
You may use the visual content enclosed in <vision> and </vision> as supporting evidence when forming your judgment. However, treat it only as a reference and verify your conclusions against the CSV data.
...
class EvalConfig:
    def __init__(
        ...
        vision_evidence_by_id: Optional[Dict[str, str]] = None,
    ) -> None:
        ...
        self.vision_evidence_by_id = vision_evidence_by_id or {}
...
class OpenAIChatGenerator:
    ...
    def respond(self, messages: List[Dict[str, str]], temperature: float, top_p: float) -> str:
...
class TableBenchReActRunner:
    ...
    def run_sample(self, raw_sample: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.interpreter = self._new_interpreter()
        vision_evidence = self.config.vision_evidence_by_id.get(sample["id"])
        ...
        if not pred_answer:
            pred_answer = extract_answer("\n".join(item["content"] for item in trajectory))
...
def load_vision_evidence(path: Path) -> Dict[str, str]:
    evidence_by_id: Dict[str, str] = {}
    fields = ("salient_observations", "visual_summary", "uncertainties")
    for row in read_jsonl(path):
        if row.get("parse_status") != "ok":
            continue
        sample_id = str(row.get("id", "")).strip()
        caption = row.get("caption")
        if not sample_id or not isinstance(caption, dict):
            continue
        if any(field not in caption for field in fields):
            continue
        selected = {field: caption[field] for field in fields}
        evidence_by_id[sample_id] = json.dumps(selected, ensure_ascii=False, indent=2)
...
def run_eval(
    ...
    vision_evidence_by_id: Dict[str, str] = {}
    if vision_file is not None:
        vision_evidence_by_id = load_vision_evidence(vision_file)
    ...
    if require_vision:
        rows = [
            row
            for row in rows
            if str(row.get("id", "")).strip() in vision_evidence_by_id
        ]
    ...
    config = EvalConfig(
        ...
        vision_evidence_by_id=vision_evidence_by_id,
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--vision_file",
        type=Path,
        help="Optional VLM caption JSONL file keyed by sample id.",
    )
    parser.add_argument(
        "--require_vision",
        action="store_true",
        help="Evaluate only samples with a valid caption in --vision_file.",
    )
    parser.add_argument(
        "--qtypes",
        nargs="+",
